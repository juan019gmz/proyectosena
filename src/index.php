...
// Consulta para obtener solo los productos con stock menor que 36
$query = mysqli_query($conexion, "SELECT p.*, t.tipo, pr.nombre FROM producto p INNER JOIN tipos t ON p.id_tipo = t.id INNER JOIN presentacion pr ON p.id_presentacion = pr.id WHERE p.existencia < 36");
...
if ($result > 0) {
    while ($data = mysqli_fetch_assoc($query)) {
        ...
        // Mostramos los datos del producto en la tabla solo si el stock es menor que 36
        if ($stock < 36) {
            echo "<script>alert('El producto " . $data['descripcion'] . " tiene un stock bajo (" . $stock . " unidades).');</script>";
        }

        echo "<tr>";
        echo "<td>" . $data['codproducto'] . "</td>";
